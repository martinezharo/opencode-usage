...
export function querySessions(db, limit = SESSION_LIMIT) {
  const sessions = db
    .prepare(
  ...
  const models = db
    ...
    .all(...sessions.map((session) => session.id), PROVIDER);

  const bySession = new Map();
  for (const row of models) {
    const list = bySession.get(row.session_id) ?? [];
    list.push({ name: row.model, cost: row.cost ?? 0, runs: row.runs });
    bySession.set(row.session_id, list);
  }

  return sessions.map((session) => {
    const sessionModels = bySession.get(session.id) ?? [];
    return {
      id: session.id,
      title: session.title,
      directory: session.directory,
      agent: session.agent,
      cost: sessionModels.reduce((sum, model) => sum + model.cost, 0),
...
export async function snapshot({
  ...
  try {
    ...
    if (db) {
      try {
        sessions = querySessions(db).map((session) => ({
          ...session,
