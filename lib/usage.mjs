...
export async function snapshot({
  ...
  try {
    ...
    allTime = consoleModels ?? localModels(queryAllTime(db));
    ...
    for (const [index, { key, label }] of WINDOWS.entries()) {
      ...
      const rows = useConsole ? null : queryModels(db, range.start, now + 1);
      const { total, models } = useConsole
        ? segmentsFrom(consoleWindow.models)
        : segmentsFrom(localModels(rows));

      const percent = ranges != null ? ranges[key].percent : (total / limit) * 100;
      const planSpent = (percent / 100) * limit;
